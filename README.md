...
> 双 SoC 分布式车载智能座舱：**RK3588 = 行车记录媒体节点 + 端侧语音节点**（V4L2 采集 / MPP 硬编 / MP4 循环存储 / RTMP 推流 / NPU 事件锁录 / NPU 流式 ASR + 板端 TTS），**Jetson Orin = 中心 AI 节点**（LLM / RAG / 意图路由 / 画面问答 / Qt 座舱）。以太网互联，视频走 RTMP、控制与状态走 ZeroMQ，全离线跨板闭环；座舱 UI 同一套代码 conf 切换部署在 Jetson 桌面或 RK 触摸屏。

**状态：核心功能全部实测完成（R1-R10、R13-R15）；2026-09-06 两轮全量验证 + 第三轮性能复测全绿**

- ✅ RK3588：采集 30.0fps → MPP 硬编（延迟 4.8ms）→ 分段循环存储 → RTMP 推流 → ZMQ 状态/事件服务 → **YOLOv5s INT8 事件锁录（NPU 25.3ms，录像满帧无损）**
- ✅ Jetson：七服务全栈一键启动 + 回归 4/4 PASS；语义双路意图路由（留出探针 15/15）；RAG 真实车主手册（390 页 PDF→577 块向量库）
- ✅ 语音推理下沉 RK：ASR fp16 三模型 NPU 三核分载（**RTF 0.26-0.30，转写与 fp32 基线逐字一致**）+ SummerTTS 板端合成（**RTF 0.22-0.27**）；真麦声学自环识别逐字正确
- ✅ 跨板闭环：语音查询/控制/抓拍全通（跨板 REQ RTT 0.73-2ms）；Qt 座舱**双端部署**（Jetson 桌面 + RK 1024x600 触摸屏，真屏验证）
- ✅ 画面问答：语音"画面里有什么"→ **Qwen2-VL-2B 端侧推理**（按需换出 LLM 的生命周期管理），应答与画面实测一致

![RK3588 触摸屏座舱——语音导航到行车记录页，实时预览 LIVE + 存储仪表环 + 事件日志 + 语音气泡](docs/images/dash_rk_dashcam_crop.png)

## 双板实物

![双板实物：左上 Jetson Orin Nano、右侧 RK3588 LubanCat-4、左下 7 寸 DSI 触摸屏（座舱 UI 运行中），两板经路由器以太网互联，麦克风/音箱 USB 组合设备接 RK](docs/images/hardware_setup.jpg)

## 系统架构

![VoxDrive 系统架构：RK3588 媒体+语音推理节点与 Jetson 中心 AI 节点，以太网互联（RTMP 视频面 / ZMQ 控制与语音面）](docs/images/arch.png)

**任务划分原则**：数据密集走专用加速器（RK 的 VPU/RGA/NPU），模型密集走中心算力（Jetson GPU）；实时敏感、算力可承担的小模型（ASR/TTS）下沉边缘，大模型（LLM/VLM）留中心。主码流 NV12 由 ISP 直出直入 MPP，RGA 不在主链路（用于检测前处理缩放与预留子码流）。
...
- **分段循环存储**：段边界严格落在 I 帧（每段独立可解码）；`statvfs` 监控磁盘水位，超阈值（默认 85%）按最旧优先删除已关闭段，**永不删正在写的当前段**；写失败立即收段，下个 I 帧自动重开——长时间录制不中断。
- **推流与存储互不牵连**：RTMP 断连时只丢非 I 帧，按"I 帧到达 + 冷却期满"节奏重连，录像管线不受影响；两路消费者挂在 `IVideoSink` 接口下，可独立启停或替换。
...
规则与语义双路意图识别（语义通路=离线意图中心余弦，阈值留出探针实测校准；规则兜底并在紧急/指令/实时车况域优先）；LLM 输出被约束为 `respond` / `tool_call` 两类 JSON，工具调用走"调用→回填→再生成"两段循环。跨板工具由 tool_bus 经上述 ZMQ 双通道访问 RK 节点。ASR 走 fp16 而非 INT8 是实测定稿：INT8 量化精度不满足流式（状态累积崩溃），fp16 零量化损失且 RTF 0.26-0.30 余量充足。

**视觉问答（VLM）**："画面里有什么"→ vlm 直通 → 抓 RK 最新帧（0.35s）→ 宿主降采样 960×540 → Qwen2-VL-2B（llama.cpp mtmd）推理作答 → 恢复原 LLM → TTS 播报。8GB 统一内存上 VLM 与全栈共存不可行，采用**按需换出**生命周期（停 llm 的 llama-server → evict 页缓存 → 起 VLM → 应答后恢复），恢复后常规查询无损。

**行车记录域走确定性直通**：关键词命中即直接执行跨板工具，LLM 只把真实工具结果组织成播报文本——不把真设备控制交给 1.5B 小模型在 7 个工具间选择（实测它会选错："关闭预览"→车窗全关、"还剩多少存储"→编数）。异常事件（水位删除/写失败/采集超时/**检测到行人车辆**）经 :6701 上行，dashboard 订阅后弹告警横幅。

**事件锁录（RK NPU）**：录像管线旁挂检测线程——每 30 帧取最新帧（忙时覆盖、不反压采集），RGA 转 RGB 送 RKNN YOLOv5s INT8（NPU core0，推理 ~25ms）检测行人/车辆；命中即锁定当前录像段（LOCK_ 前缀豁免循环覆盖，配额超限自动释放最旧），事件同步上行 Jetson 告警。
