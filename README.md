...
> 双 SoC 分布式车载智能座舱：**RK3588 作为行车记录媒体节点**（V4L2 摄像头采集 / MPP 硬件编码 / MP4 分段循环存储 / RTMP 推流 / **RKNPU 事件锁录**），**Jetson Orin 作为端侧 AI 语音节点**（流式 ASR / LLM / RAG / TTS / Qt 座舱界面）。两板以太网互联，视频流走 RTMP、控制与状态走 ZeroMQ，实现"语音控制行车记录、画面预览、录像与存储状态查询"的跨节点闭环；座舱 UI 同一套代码可部署在 Jetson 桌面或 RK 触摸屏（conf 驱动，通信拓扑不变）。

**状态：核心功能全部实测完成（R1-R10、R13）；ASR/TTS 推理下沉 RK NPU（R14）进行中**
...
- 🚧 R14：ASR/TTS 推理 RKNN INT8 量化迁移（转换管线全通，流式精度优化中）
