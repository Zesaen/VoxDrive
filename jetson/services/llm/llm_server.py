...
7. dashcam: 行车记录仪（跨板真实设备）。动作：status=查询录像/存储/预览状态，
   ...
   snapshot=抓拍一张照片并保存。
   用户问"还剩多少存储/存储空间/现在录着吗/录像状态/记录仪怎么样"必须用 dashcam 的 status，
   禁止用 sensor_read 或 camera_capture 回答记录仪问题；说"拍张照/拍照/抓拍"用 snapshot；
   要求开始或停止录像/预览用对应开关动作。
"""

# R6 TODO：本表与 intent_router 规则表/TOOL_SPEC 三处重复，届时合一为单一工具注册表
FALLBACK_TOOL_RULES = [
    ("dashcam", "status", ["行车记录", "记录仪", "存储", "录着", "在录像"]),
