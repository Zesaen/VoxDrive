...
# 日志格式由调用方（rag_server → vox_log）统一配置，此处不劫持 root logger
logger = logging.getLogger(__name__)


def _resolve_sentence_transformer_device() -> str:
    # Keep the embedding model off the Jetson GPU by default so the LLM can use VRAM.
    return os.environ.get("RAG_EMBEDDING_DEVICE", "cpu")
...
class VehicleVectorSearch:
    ...
    def load_model(self, model_name):
        ...
        device = _resolve_sentence_transformer_device()
